...
if (!file.exists(paste0(path, "/model_file.Rds"))){
  rmarkdown::render("wave2-fit.Rmd", params = list(positive_to_hospitalized_csv=paste0(path, "/msm_fit/final_pos_for_model.csv"), 
                    ...
                    model_cache_file=paste0(path, "/model_file.Rds"), use_15_9_reduced_model=TRUE),
...
list(n_cores = 32L, n_monte_carlo_runs = 1000L, max_path_length = 14L,
    ...
    analysis_date = analysis_date, predict_n_days_ahead = 100,
    seed = strtoi(42), 
    output_dir = paste0(path, "/out/"),
    positive_with_history_input_csv = NULL,
    positive_no_history_input_csv = NULL,
    hospitalized_with_history_input_csv = NULL,
    hospitalized_no_history_input_csv = paste0(path, "/death_input.csv"),
    true_counts_csv = paste0(path, "/death_true.csv"),
    prefix = 'death_group_')
...
if (!is.null(POSITIVE_WITH_HISTORY_INPUT_CSV)) {
  ...
  #formattable(head(positive_with_history_df, n=5))
...
if (!is.null(POSITIVE_NO_HISTORY_INPUT_CSV)) {
  ...
  #formattable(head(positive_with_NO_history_df, n=5))
...
if (!is.null(HOSPITALIZED_WITH_HISTORY_INPUT_CSV)) {
  ...
  #formattable(head(hospitalized_with_history_df, n=5))
...
if (!is.null(HOSPITALIZED_NO_HISTORY_INPUT_CSV)) {
  ...
  #formattable(head(hospitalized_with_NO_history_df, n=5))
  ...
  hospitalized_with_NO_history_patient_objects = lapply(1:nrow(hospitalized_with_NO_history_df), function(row_idx) {
    ...
    covariates_at_entry_to_state_at_t_start = construct_hospitalized_patient_covariates(
      ...
      is_male = row$is_male, 
      state_at_hospitalization = row$state_at_hospitalization, 
      cumulative_time_since_hospitalization = clip(as.integer(T-as.Date(row$date_of_hospitalization)), 0),
      was_severe = row$was_severe, # 0,
      was_critical = row$was_critical # 0,
    )
    
    list(
      t_start = row$date_of_hospitalization,
      covariates_at_t_start = covariates_at_entry_to_state_at_t_start,
      cumulative_time_at_t_start = clip(as.integer(T-as.Date(row$date_of_hospitalization)), 0),
...
## ------------------------------------------------------------------------------------------------------------
# start_time <- Sys.time()
# if (TEST_RUN) all_patients = all_patients[1:100]
...
expected_deaths_by_days_since_hospitalization(cache_dir, T, PREDICT_N_DAYS_AHEAD, OUTPUT_DIR)
